class SummaryCollectionsController < ApplicationController
  ...
  def index
    ...
    if params['ar_account_id'].present?
      @customer_ledger_deposited = @customer_ledger_deposited.where(ar_account_id: params['ar_account_id'].to_i, is_deposited: true)
    end
    if params[:up].present?
      @customer_ledger_deposited = @customer_ledger_deposited.where("deposited_date BETWEEN ? AND ? AND is_deposited = true", params[:up][:start_date].to_date, params[:up][:end_date].to_date)
    else
